...
var s = { name: "auth0-spa-js", version: "2.3.0" };
var c = () => Date.now();
...
var d = class _d extends u {
  constructor(e2, t2, i2, o2 = null) {
    super(e2, t2), this.state = i2, this.appState = o2, Object.setPrototypeOf(this, _d.prototype);
  }
};
...
async function T(t2, i2) {
  ...
  const h2 = "urn:ietf:params:oauth:grant-type:token-exchange" === l2.grant_type, p2 = Object.assign(Object.assign(Object.assign({}, l2), h2 && a2 && { audience: a2 }), h2 && r2 && { scope: r2 }), g2 = d2 ? v(p2) : JSON.stringify(p2);
  return await async function(t3, i3, o3, n3, a3, r3, s2) {
    ...
    if (d3) throw d3;
    const l3 = c3.json, { error: h3, error_description: p3 } = l3, g3 = e(l3, ["error", "error_description"]), { ok: w2 } = c3;
    if (!w2) {
      const e2 = p3 || `HTTP error. Unable to fetch ${t3}`;
      if ("mfa_required" === h3) throw new m(h3, e2, g3.mfa_token);
      if ("missing_refresh_token" === h3) throw new f(o3, n3);
      throw new u(h3 || "request_error", e2);
    }
    return g3;
  }(`${o2}/oauth/token`, n2, a2 || "default", r2, { method: "POST", body: g2, headers: { "Content-Type": d2 ? "application/x-www-form-urlencoded" : "application/json", "Auth0-Client": btoa(JSON.stringify(c2 || s)) } }, i2, d2);
}
var j = (...e2) => {
...
var z = class _z {
  ...
  static fromKey(e2) {
    ...
    return new _z({ clientId: i2, scope: n2, audience: o2 }, t2);
  }
  static fromCacheEntry(e2) {
    const { scope: t2, audience: i2, client_id: o2 } = e2;
    return new _z({ scope: t2, audience: i2, clientId: o2 });
  }
};
var C = class {
  set(e2, t2) {
...
var te = class {
  ...
  async loginWithPopup(e2, t2) {
    ...
    const n2 = await ((e3) => new Promise((t3, i3) => {
      ...
      o3 = function(r2) {
        if (r2.data && "authorization_response" === r2.data.type) {
          if (clearTimeout(a3), clearInterval(n3), window.removeEventListener("message", o3, false), e3.popup.close(), r2.data.response.error) return i3(u.fromPayload(r2.data.response));
          t3(r2.data.response);
        }
    ...
    if (o2.state !== n2.state) throw new u("state_mismatch", "Invalid state");
  ...
  async handleRedirectCallback(e2 = window.location.href) {
    ...
    if (!r2) throw new u("missing_transaction", "Invalid state");
    if (this.transactionManager.remove(), n2) throw new d(n2, a2 || n2, i2, r2.appState);
    if (!r2.code_verifier || r2.state && r2.state !== i2) throw new u("state_mismatch", "Invalid state");
  ...
  async _getTokenFromIFrame(e2) {
    ...
    try {
      ...
      const p2 = await ((e3, t3, i4 = 60) => new Promise((o3, n3) => {
      ...
      if (n2 !== p2.state) throw new u("state_mismatch", "Invalid state");
      const m2 = await this._requestToken(Object.assign(Object.assign({}, e2.authorizationParams), { code_verifier: r2, code: p2.code, grant_type: "authorization_code", redirect_uri: s2, timeout: e2.authorizationParams.timeout || this.httpTimeoutMs }), { nonceIn: a2, organization: t2.organization });
  ...
  async exchangeToken(e2) {
    return this._requestToken({ grant_type: "urn:ietf:params:oauth:grant-type:token-exchange", subject_token: e2.subject_token, subject_token_type: e2.subject_token_type, scope: j(e2.scope, this.scope), audience: e2.audience || this.options.authorizationParams.audience });
  }
};
var ie = class {
};
